...
vec3   m_transparency(0.5, 0.5, 0.5);
...
//Find the closest intersection with the index E spheres along the ray 
int findIntersection2(const Ray& ray, vec3& p, vec3& n, int E)
{
	//Find the closest intersection within [ray.p0, ray.p1]
	int      iSphere = -1;
	float   T = 1.0;   //The camera faces the negative z-axis;

	vec3 center = vec3(viewModel * vec4(center_world[E], 1));

	vec3 p_i, n_i;

	vec3 p10 = (normalize(ray.p1 - ray.p0)) * 1.0e-3f;

	Ray tempRay(ray.p0 + p10, ray.p1);

	float t = findIntersection(tempRay, center, radius[E], p_i, n_i);

	if (t <= T && t > 0) { iSphere = E; T = t; p = p_i; n = n_i; }
...
//Calculate refract translucent
vec3 refractTransparency(const vec3& ray, const vec3& n)
{
	float cos_i = dot(normalize(ray), n);

	float eta_i = 1, eta_r = 1.5;

	vec3 tempN = n;

	if (cos_i < 0) {
		cos_i = -cos_i;
		float   tempEta_i = eta_i;
		eta_i = eta_r;
		eta_r = tempEta_i;
		tempN = -n;
	}

	float eta = eta_i / eta_r;
	float cos_r = 1 - eta * eta * (1 - cos_i * cos_i);

	if (cos_r < 0) {
		return vec3(0);
	}
	else {
		return (eta * cos_i - sqrt(cos_r)) * tempN - eta * ray;
	}
}
...
vec3 intensity(const Ray& ray, const Light l[], int nLights, int depth, int E = -1, bool outside = true)
{
	...
	if (outside == true) 
		iObject = findIntersection(ray, p, n, E);
	else
		iObject = findIntersection2(ray, p, n, E);
	...
	if (iObject != -1)   //Hit an object
	{
		...
		if (depth < DEPTH)
		{
			if (selection == 1) {	// Reflective(not translucent) spheres
				//Reflection ray
				vec3	r = normalize(reflect(ray.p0 - ray.p1, n));
				vec3	pr = p + 1.0E10f * r;   //Point far awary from p along r
				Ray		recursiveRay(p, pr);

				vec3	I_R = intensity(recursiveRay, l, nLights, depth + 1, iObject, outside);

				for (int i = 0; i < 3; i++)
					I[i] += m_specular[i] * I_R[i];
			}
			else if (selection == 2) {		// Translucent(not reflective) spheres without refraction
				//Transmission ray without refraction
				Ray     tempRay(p, ray.p1);
				vec3	pt, nt;      //Position and normal in the eye coordinate system
				int     itObject = findIntersection2(tempRay, pt, nt, iObject);

				Ray     recursiveRay(pt, ray.p1);

				vec3	I_T = intensity(recursiveRay, l, nLights, depth + 1, iObject, outside);

				for (int i = 0; i < 3; i++)
					I[i] += m_transparency[i] * I_T[i];
			}
			else if (selection == 3) {		// Translucent(not reflective) spheres with refraction
				//Transmission ray with refraction
				vec3	v1 = normalize(refractTransparency((ray.p0 - ray.p1), n));
				vec3	pStart1 = p + 1.0e-3f * v1;
				vec3	pEnd1 = p + 1.0E10f * v1;
				Ray		ray1(pStart1, pEnd1);

				vec3	pt, nt;      //Position and normal in the eye coordinate system
				int		itObject = findIntersection2(ray1, pt, nt, iObject);

				vec3	v2 = normalize(refractTransparency((ray1.p0 - ray1.p1), nt));
				vec3	pStart2 = pt + 1.0e-3f * v2;
				vec3	pEnd2 = pt + 1.0E10f * v2;
				Ray		ray2(pStart2, pEnd2);

				vec3	I_T = intensity(ray2, l, nLights, depth + 1, itObject, outside);

				for (int i = 0; i < 3; i++)
					I[i] += m_transparency[i] * I_T[i];
			}
			else if (selection == 4) {		// Reflective and refractive spheres without reflections inside spheres
				//Reflection ray
				vec3	r = normalize(reflect(ray.p0 - ray.p1, n));
				vec3	pr = p + 1.0E10f * r;   //Point far awary from p along r
				Ray		recursiveRay(p, pr);
			...
			else if (selection == 5) {		// Reflective and refractive spheres with reflections inside spheres
				if (outside) { //Ray is located in outside
					//Reflection ray
					vec3	r = normalize(reflect(ray.p0 - ray.p1, n));
					vec3	pr = p + 1.0E10f * r;   //Point far awary from p along r
					Ray		recursiveRay(p, pr);
					...
					//Reflection at inside spheres
					vec3	r_in = normalize(reflect(ray1.p0 - ray1.p1, nt));		// Reflection vector at inside sphere
					vec3	pr_in = pt + 1.0E10f * r_in;							
					pt = pt + 1.0e-3f * r_in;
					Ray		insideRay(pt, pr_in);

					vec3	I_in = intensity(insideRay, l, nLights, depth + 1, iObject, !outside);

					for (int i = 0; i < 3; i++)
						I[i] += m_specular[i] * I_in[i];
				}
				else if (outside == false){  //Ray is located in inside
					//Reflection ray
					vec3	r = normalize(reflect(ray.p0 - ray.p1, n));		// Reflection vector at inside sphere
					...
					vec3	I_T = intensity(ray1, l, nLights, depth + 1, iObject, !outside);
...
#pragma omp master
	...
	cout << "Keyboard input: shift + up/down for ray tracing depth" << endl;
	cout << endl;
	cout << "Keyboard input: 1 for reflective(not translucent) spheres" << endl;
	cout << "Keyboard input: 2 for translucent(not reflective) spheres without refraction" << endl;
	cout << "Keyboard input: 3 for translucent(not reflective) spheres with refraction" << endl;
	cout << "Keyboard input: 4 for reflective and refractive spheres without reflections inside spheres" << endl;
	cout << "Keyboard input: 5 for reflective and refractive spheres with reflections inside spheres" << endl;
}
...
int main(int argc, char* argv[]) {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (elapsed > timeStep) {
			...
			elapsed = 0;   //Request the elapsed time
		}
...
void keyboard(GLFWwindow* window, int key, int scanmode, int action, int mods)
{
	...
	if ((action == GLFW_PRESS || action == GLFW_REPEAT) && (mods & GLFW_MOD_SHIFT))
	{
		switch (key)
		{
		case GLFW_KEY_UP:      if (DEPTH < 9) DEPTH++;   break;
		case GLFW_KEY_DOWN:      if (DEPTH > 1) DEPTH--; break;
		}
	}
}