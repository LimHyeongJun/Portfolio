...
void keyboard(GLFWwindow* window, int key, int scanmode, int action, int mods)
{
	...
	if ((action == GLFW_PRESS || action == GLFW_REPEAT) && (mods & GLFW_MOD_SHIFT))
	{
		switch (key)
		{
			//Navigation
		case GLFW_KEY_LEFT: {
			float distanceAandE = sqrt(pow(C.a.x - C.e.x, 2) + pow(C.a.y - C.e.y, 2) + pow(C.a.z - C.e.z, 2));
			glm::mat4 M(1.0);
			M = glm::rotate(M, glm::radians(10.0f), C.u);

			vec4 tempF = vec4(C.f, 1.0);
			tempF = M * tempF;

			C.f = normalize(vec3(tempF));
			C.r = normalize(cross(C.f, C.u));
			C.a = C.e + (C.f * distanceAandE);
