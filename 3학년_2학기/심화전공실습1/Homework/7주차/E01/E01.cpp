...
void init()
{
	...
	cout << endl;
	cout << "Keyboard Input: 1 for draw normal vectors of triangles" << endl;
	cout << "Keyboard Input: 2 for draw normal vertors of vertices" << endl;
}
...
//Draw normal vectors of triangles using line segments
void drawNormalOfTriangles()
{
	vec3* temp = new vec3[nFaces];
	for (int i = 0; i < nFaces; i++) {
		temp[i] = vertex[face[0][i]] + vertex[face[1][i]] + vertex[face[2][i]];
		temp[i].x = temp[i].x / 3.0f;	temp[i].y = temp[i].y / 3.0f;	temp[i].z = temp[i].z / 3.0f;
	}

	vec3* endPoint = new vec3[nFaces];
	for (int i = 0; i < nFaces; i++) {
		//point + vector = point
		//temp is point, fnormal is vector
		endPoint[i] = temp[i] + fnormal[i];
	}
	
	glBegin(GL_LINES);
	for (int i = 0; i < nFaces; i++) {
		glVertex3fv(value_ptr(temp[i]));
		glVertex3fv(value_ptr(endPoint[i]));
	}
	glEnd();
}

//Draw normal vectors of vertices using line segments
void drawNormalOfVertices()
{
	vec3* endPoint = new vec3[nVertices];
	for (int i = 0; i < nVertices; i++) {
		//point + vector = point
		endPoint[i] = vertex[i] + vnormal[i];
	}
...
void render(GLFWwindow* window)
{
	...
	switch (selection)
	{
	case 1: drawNormalOfTriangles(); 						break;
	case 2: drawNormalOfVertices();	 						break;
	case 3: drawNormalOfTriangles(); drawFlatMesh();		break;
	case 4: drawNormalOfVertices();	 drawSmoothMesh();		break;
	}
}
