...
void init()
{
	...
	cout << "Keyboard Input: space for rotate on/off" << endl;
	cout << endl;
}
...
bool readMesh(const char* filename)
{
	...
	//Extract edges
	edgeList = new list<int>[nVertices];
	for (int i = 0; i < nFaces; i++) {
		int a = face[0][i], b = face[1][i], c = face[2][i];

		a < b ? edgeList[a].push_back(b) : edgeList[b].push_back(a);
		b < c ? edgeList[b].push_back(c) : edgeList[c].push_back(b);
		c < a ? edgeList[c].push_back(a) : edgeList[a].push_back(c);
	}

	for (int i = 0; i < nVertices; i++) {
		edgeList[i].sort();
		edgeList[i].unique();
		nEdges += (int)edgeList[i].size();
	}

	cout << "# of edges = " << nEdges << endl;
...
void drawMesh()
{
	...
	for (int i = 0; i < nVertices; i++) {
		...
		for (it = edgeList[i].begin(); it != edgeList[i].end(); ++it) {
